...
protocol SmsApiRepository: ApiRepository {
    ...
    func smsVerification(phone: String) -> AnyPublisher<ServerResponse<JsonSMSVerification>, Error>
    func smsValidation(phone: String) -> AnyPublisher<ServerResponse<JsonSMSValidation>, Error>
...
struct PandasazaSmsApiRepository: SmsApiRepository {
    ...
    func verifyingSMS(phone: String) -> AnyPublisher<Bool, Error> {
        let smsRequest: AnyPublisher<ServerResponse<JsonSMSVerification>, Error> = request(endpoint: API.smsVerification(phone))
        return smsRequest.map { return $0.data.registered }.eraseToAnyPublisher()
    }
    
    func requestSMSValidation(phone: String) -> AnyPublisher<String, Error> {
        let smsRequest: AnyPublisher<ServerResponse<JsonSMSValidation>, Error> = request(endpoint: API.smsValidation(phone))
